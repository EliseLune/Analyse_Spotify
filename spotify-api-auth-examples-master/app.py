...
@app.route('/choix')
def choix():
    '''Choix de la playlist à analyser'''
    ...
    res_data = res.json()
    user_id=res_data["id"]
    playlists_id = get_playlist_id(user_id)
    ...
    return render_template('choix.html', playlists=playlists_id, sp=sp)

@app.route('/analyse/<playlist_id>')
def analyse(playlist_id):
    table = creat_df_audiofeatures(playlist_id)
    bar = create_plot(table)
    return render_template('analyse.html', plot=bar)

# print("La la", user_id, get_playlist_id(user_id), file=sys.stderr)
# Comment afficher quelque chose sur la console